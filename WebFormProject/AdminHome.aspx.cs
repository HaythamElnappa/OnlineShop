    ...
    public partial class WebForm1 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    pormotionlist.InnerHtml += $"<li class=\"h6 text-left \"><a href=\"#\">{row["Name"].ToString()}</a> </li>";

                }
                DataTable fillfooter2 = Product.SelectLastfiveProdeuct();
                foreach (DataRow row in fillfooter2.Rows)
                {
                    lastprouduect.InnerHtml += $"<li class=\"h6 text-left \"><a href=\"#\">{row["Name"].ToString()}</a> </li>";
                ...
                {
                    {
                        ...
                        cat1.InnerHtml += $"<div class=\"cards  \"><div class=\"card  align-content-center wow slideInLeft\" data-wow-duration=\"1s\" data-wow-delay=\"1s\">{htmlimg}<h2  class=\"text-nowrap lighttext\">{row["name"].ToString()}</h2><p class=\"text-nowrap text-white\">{row["Description"].ToString()}</p><a href=\"#\" class=\"btn btn-secondary btn-success\">DETALES</a></div>";
                ...
                //{
                //    catdropdownlist.InnerHtml += $"<a class=\"dropdown-item  text-white\" href=\"#\">{row["Name"].ToString()}</a>";
                //}
